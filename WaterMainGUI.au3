...
#EndRegion PRE-GUI INITIALIZATION
...
Global $reminder5, $reminder6, $reminder7, $reminder8, $reminder9, $reminder10, $debugMode, $iniMinimize
#EndRegion GLOBALS
...
$labTotals = GUICtrlCreateLabel($currentIntake & "/" & $dailyIntake & "oz.", 30, 82, 120, 40, $SS_CENTER)
GUICtrlSetFont(-1, 18, 400, $GUI_FONTNORMAL, "", $ClEARTYPE_QUALITY)
$grpQuickAdd = GUICtrlCreateLabel("Quick Add", 34, 132, 60, 20)
GUICtrlSetFont(-1, 9, 500, $GUI_FONTNORMAL, "", $CLEARTYPE_QUALITY)
$inQuickAdd = GUICtrlCreateInput("", 32, 152, 57, 21, BitOR($ES_CENTER, $ES_NUMBER))
GUICtrlSetFont(-1, 9, 500, $GUI_FONTNORMAL, "", $CLEARTYPE_QUALITY)
GUICtrlSetLimit(-1, 3, 0)
$labOz = GUICtrlCreateLabel("oz.", 94, 158, 18, 17)
GUICtrlSetFont(-1, 9, 500, $GUI_FONTNORMAL, "", $CLEARTYPE_QUALITY)
$butQuickAdd = GUICtrlCreateButton("Add", 115, 144, 50, 33)
GUICtrlSetFont(-1, 9, 500, $GUI_FONTNORMAL, "", $CLEARTYPE_QUALITY)
...
#Region GUI_OPTIONS
$gui_options = GUICreate("Options", 169, 145, -1, -1)
...
GUICtrlSetFont(-1, 9, 500, $GUI_FONTNORMAL, "", $CLEARTYPE_QUALITY)
$butSave = GUICtrlCreateButton("Save", 26, 110, 114, 25)
GUICtrlSetFont(-1, 9, 500, $GUI_FONTNORMAL, "", $CLEARTYPE_QUALITY)
GUISetIcon(@ScriptDir & "\favicon.ico")
GUISetState(@SW_HIDE)
   ...
   Case $GUI_EVENT_CLOSE
	  ...
	  Else
		 If $iniMinimize = "True" Then
			...
			ToolTip("WaterTracker minimized to tray." & @CRLF & "Left click tray icon to open.", @DesktopWidth-190, @DesktopHeight-80, "", $TIP_BALLOON)
...
Func create_ini() ;creates ini file. used on first-time startup or if ini is not detected.
   ...
   FileWrite($testfile, "dailyval=100" & @CRLF)
   FileWrite($testfile, "dailylogtime=01:00 AM" & @CRLF)
   FileWrite($testfile, "logging=False" & @CRLF)
   FileWrite($testfile, "minimize=False" & @CRLF)
   FileWrite($testfile, "remindsound=False" & @CRLF)
   FileWrite($testfile, "remindsound=False" & @CRLF)
...
Func testfunc()
   GetTime()
;~    ConsoleWrite("$idDate = " & GUICtrlRead($idDate) & @CRLF)
;~    ConsoleWrite("$longtime = " & $longtime & @CRLF)
   GUICtrlSetData($idDate, "12:12")
EndFunc
